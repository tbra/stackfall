...
Project Claude skills are versioned: bump the `Version:` line for each behavior change, run `python tools/skill_versions.py` at session start, and verify/announce the on-disk version on each use. A stale loaded skill must be re-invoked or run in a fresh session. Auto-run has no fixed task-count cutoff: keep main-thread output small, checkpoint at batch boundaries, and continue through native compaction when recovery is safe.
...
**Beads and context:** the orchestrator owns claims, assignments, acceptance and closure; workers comment on their assigned issue only. Every Beads write passes `--actor <agent-name>` (`stackfall-orchestrator` for the main Claude session). Set `--assignee "<worker profile> (<model>)"` at dispatch, or `stackfall-orchestrator` for self-work; unstarted issues stay unassigned and no issue closes unassigned. Workers return compact, self-contained JSON with candidate, files, named checks, decisive finding and next action. Their Bead comment is for recovery, not routine rereading; use comment-free `bd show <id> --json --brief-deps` for ordinary issue reads. After a delivered batch, checkpoint the next action and refresh Claude context if stale output has accumulated. This project-specific rule supersedes generic `bd prime` and managed-block worker-close instructions; see `docs/AGENT_WORKFLOW.md`.
