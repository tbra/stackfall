...
**Beads ownership:** the orchestrator serializes shared Beads writes and accepts/closes issues; workers return checkpoint and completion evidence. Every agent Beads write passes `--actor <agent-name>` (the orchestrator uses `--actor claude-orchestrator`); without it `bd` stamps the git user and the owner cannot tell agent writes from their own. This project-specific coordination rule takes precedence over generic skill, `bd prime`, and generated-block instructions telling each worker to mutate or close its own issue.
