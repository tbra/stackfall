...
Project Claude skills are versioned: bump the `Version:` line for each behavior change, run `python tools/skill_versions.py` at session start, and verify/announce the on-disk version on each use. A stale loaded skill must be re-invoked or run in a fresh session. Auto-run has a fixed unattended cycle cap; the project `autoCompactWindow` setting provides native early compaction but does not replace that cap.
