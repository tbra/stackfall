...
## Note for P1/integrator: a stub compile bug fixed in passing
...
warnings-as-errors setting ("overrides a method from native class Object"). P2 renamed
it to `circle_is_connected` (and updated the one doc-comment cross-reference) purely to
unblock every package's build — this is not a P2 ownership change, and P1's own agent
should feel free to rename it again once the real solver lands.
