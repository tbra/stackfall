...
- **Report template:** `Bead | verdict | elapsed | files | checks (command, counts, result) | evidence paths | blocker/decision | next action`. Keep clean results to a few lines. Put full logs in the supplied scratchpad; include decisive error lines only when a check fails. The orchestrator records the durable checkpoint in Beads.
