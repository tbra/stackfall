...
The orchestrator runs `bd prime`, examines `bd ready` and `bd list --status=in_progress`, and reads each assigned issue with `bd show <id>`. It claims the issue once, records the assigned worker, and serializes Beads writes (this project uses embedded Dolt). Every such write carries `--actor stackfall-orchestrator` (owner request 2026-09-22: agent writes must be distinguishable from the owner's; Codex uses `--actor codex`). Workers report checkpoints to the orchestrator; they do not initialize private Beads databases inside worktrees. A replacement session reads the existing records before reclaiming anything, and verifies that the old worker is no longer active.
