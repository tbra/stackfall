...
Iterate with the targeted runner (own files + the affected scripts the brief names). The full suite is NOT a per-package gate: the orchestrator runs it once per merged batch, in the background after the commit, never concurrently with another Godot process (owner decision 2026-09-21: the ~25 min suite had not caught anything the targeted sets missed). Run the multiplayer checks for network changes and the relevant physics/territory benchmarks when those systems change. Existing pending tests are limitations, not passes. For real hardware, record explicit owner steps and what remains unverified. Do not label frame-rate acceptance verified by a headless run alone. Review the exact candidate that was tested; changes after review need proportionate revalidation.
