...
- **Windowed probes (owner, 2026-09-23):** agent-launched windowed Godot runs (screenshot probes, smoke runs) must start off-screen — always pass `--windowed --position 10000,10000`, never `--always-on-top`/`--maximized` — and quit as soon as the capture is saved; use `--headless` whenever no screenshot is needed. They were popping up on the owner's other monitor.
