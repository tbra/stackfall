...
- Expected Beads comment: files changed, tests actually run, findings, decisions, unresolved work and next action. Expected handback: the five-field compact JSON pointer.
...
- **Handback:** return only one JSON object with nonempty string fields `bead`, `verdict`, `checks`, `evidence`, `next`; `evidence` starts `bd:<id>` after the comment. A success/review handback is at most 600 characters; partial/blocked/failed at most 900. If the Beads comment failed, use `verdict=blocked`, `evidence=comment-failed:<error>` and up to 3200 characters to avoid losing findings. The project hooks check this before `SubagentHandback` and once at `SubagentStop` for plain text returns. No markdown fence, large logs, or repeated report in the main chat.
