...
- Git authority for this assignment (workers default to no commits, merges, pushes or remote sync; the orchestrator owns verified delivery).
...
- **Report template:** `Bead | verdict | elapsed | files | checks (command, counts, result) | evidence paths | blocker/decision | next action`. Final handback limit: 900 characters on success, 1500 on failure. Put full logs and reasoning in the supplied scratchpad; include only the decisive error line when a check fails. The orchestrator records the durable checkpoint in Beads. A long JSON-encoded worker response still costs main-chat context.
...
The orchestrator uses `bd prime` only when Beads workflow context is missing or stale, runs `python tools/board_brief.py` for a bounded ready/in-progress overview, and reads only the next assigned issue with `bd show <id>`. Do not echo full-board JSON or large diffs/logs into the main chat. The project SessionStart hook prints only a resume pointer, avoiding a full memory dump on every new chat. The orchestrator claims an issue once, records the assigned worker, and serializes Beads writes (this project uses embedded Dolt). Every such write carries `--actor stackfall-orchestrator` (owner request 2026-09-22: agent writes must be distinguishable from the owner's; Codex uses `--actor codex`). The dispatch write also sets `--assignee "<worker profile> (<model>)"` so the record shows who did the work; orchestrator-owned packages are assigned to `stackfall-orchestrator`; issues not yet started stay unassigned (no placeholder assignees), and no issue is closed without an assignee (owner, same day). Workers report checkpoints to the orchestrator; they do not initialize private Beads databases inside worktrees. A replacement session reads the existing records before reclaiming anything, and verifies that the old worker is no longer active.
