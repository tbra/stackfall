...
4. Update a single short handoff memory with the next ready bead ID, why it is next, exact checkout/revision and anything a replacement must not undo or rediscover. Use `bd remember` for reusable lessons only; do not make another task ledger or append session history to `docs/CLAUDE_HANDOFF.md`. If there is no clear next bead, say why.
