...
2. Run `python tools/board_brief.py` for a bounded ready/in-progress overview; query `bd show <id>` only for the next named bead and its comments. Never print a whole-board JSON dump into the main chat. Check `git status --short`, current revision, worktrees and worker liveness. The live board and checkout win over dated memory. Do not reclaim a live worker or discard its partial checkout.
