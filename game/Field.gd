...
## Reason tag reported on Events.block_removed when the kill plane catches a
## body. Another M2 branch promotes this to Events.REASON_KILL_PLANE; until
## that lands, the M1 literal stays put so the two never disagree.
const REASON_KILL_PLANE: String = "kill_plane"
...
func _collect_in_disk_cells(cell_grid: CellGrid) -> PackedInt32Array:
	...
	for cy: int in range(cell_grid.res):
		for cx: int in range(cell_grid.res):
			if cell_grid.is_in_disk(cx, cy):
				cells.append(cell_grid.cell_index(cx, cy))
	return cells
...
func _on_kill_plane_body_entered(body: Node3D) -> void:
	...
	if rigid_body == null:
		return
	Events.block_removed.emit(rigid_body, REASON_KILL_PLANE)
